...
const TweetCard: React.FC<{ text: string; onCopy: () => void; }> = ({ text, onCopy }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex gap-4 items-start transition-colors">
            <div className="flex-shrink-0 w-8 h-8 bg-black dark:bg-gray-900 rounded-full flex items-center justify-center text-white">
                <XIcon className="w-4 h-4" />
            </div>
            <div className="flex-grow">
                <p className="whitespace-pre-wrap text-gray-800 dark:text-gray-200 font-sans">{text}</p>
            </div>
            <button 
    ...
    return (
        <div className="mt-6 space-y-4">
            <div className="flex justify-between items-center">
                <h4 className="font-bold text-lg text-gray-800 dark:text-white">Generated X Thread</h4>
                <button
                    onClick={handleCopyAll}
                    className="flex items-center gap-2 text-sm font-semibold text-white px-4 py-2 rounded-lg bg-gray-800 hover:bg-black dark:bg-gray-700 dark:hover:bg-gray-600 transition-all shadow-md"
                >
...
const ToolView: React.FC<ToolViewProps> = ({ tool, creationContext, setCreationContext, onNavigateToTool, onSaveAsProject, tools }) => {
  ...
  const renderNextSteps = () => {
    ...
    const NextStepCard: React.FC<{
        ...
        <button
            type="button"
            onClick={onClick}
            className="w-full text-left p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-brand-red dark:hover:border-brand-red hover:shadow-lg transition-all flex items-center gap-4 group"
        >
            <div className="flex-shrink-0 w-10 h-10 bg-gray-100 dark:bg-gray-900 rounded-lg flex items-center justify-center text-gray-500 dark:text-gray-400 group-hover:bg-red-100 dark:group-hover:bg-red-900/30 group-hover:text-brand-red transition-colors">
                {icon && React.cloneElement(icon as React.ReactElement<{ className?: string }>, { className: "w-6 h-6"})}
            </div>
            <div>
                <h6 className="font-bold text-gray-800 dark:text-gray-200 group-hover:text-brand-red transition-colors">{title}</h6>
    ...
    return (
        <div className="mt-8 p-6 bg-gray-50/70 dark:bg-gray-800/50 rounded-2xl border border-gray-200/80 dark:border-gray-700">
            <h5 className="font-bold text-xl text-center text-gray-800 dark:text-gray-200 mb-2">🚀 What's Next?</h5>
             <div className="text-center text-sm text-green-800 dark:text-green-300 mb-6 flex items-center justify-center gap-2 font-semibold bg-green-100 dark:bg-green-900/30 p-2 rounded-lg">
    ...
    const renderGeneratedList = (items: string[]) => (
         <div className="mt-6">
            ...
            {items.map((item, index) => 
                <li key={index} className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm flex justify-between items-center gap-4">
                <p className="text-gray-700 dark:text-gray-300 flex-grow">{item}</p>
                </li>
            )}
            </ul>
        </div>
    ...
    const renderGeneratedText = (text: string, title: string) => (
        <div className="mt-6">
            ...
            <div className="whitespace-pre-wrap text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 font-mono text-sm shadow-sm max-h-96 overflow-y-auto">{text}</div>
        </div>
    );
            ...
            <div key={questionKey}>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{question}</label>
                 <div className="relative w-full mt-1">
                    ...
                    <input
                        type="text"
                        className="peer w-full px-3 py-2 pl-10 bg-gray-50/70 dark:bg-gray-800/50 border-2 border-transparent rounded-lg focus:ring-2 focus:ring-brand-red/70 focus:border-transparent focus:bg-white dark:focus:bg-gray-800 transition-all shadow-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 sm:text-sm"
    ...
    const renderTitleGenerationResponse = (content: TitleGenerationResponse) => (
        <div className="mt-6 space-y-6">
            <div>
                <h4 className="font-bold text-lg text-gray-800 dark:text-white mb-3">🏆 Best Title</h4>
                <div className="p-4 bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-800 rounded-lg shadow-md">
                    <p className="text-xl font-bold text-green-900 dark:text-green-300">{content.best_title.text}</p>
                    <p className="text-sm text-green-700 dark:text-green-400 mt-2">{content.best_title.reason}</p>
                </div>
            </div>
             <div>
                <h5 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">💡 Thumbnail Text Idea</h5>
                <p className="text-gray-600 dark:text-gray-400 p-3 bg-gray-50 dark:bg-gray-800 rounded-md border dark:border-gray-700 text-sm font-mono">"{content.notes}"</p>
            </div>
            <div>
                <h4 className="font-bold text-lg text-gray-800 dark:text-white mb-3">⭐ Top Picks</h4>
                <div className="space-y-3">
                    {content.top_picks.map((pick, index) => (
                        <div key={index} className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
                            <p className="font-semibold text-gray-800 dark:text-gray-200">{pick.text}</p>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{pick.reason}</p>
                        </div>
                    ))}
                </div>
            </div>
            <details className="bg-gray-50/50 dark:bg-gray-800/50 p-4 rounded-lg border dark:border-gray-700">
                <summary className="font-semibold text-gray-700 dark:text-gray-300 cursor-pointer">Show All {content.titles.length} Title Options</summary>
                <div className="mt-4 space-y-3">
                    {content.titles.map((title, index) => (
                        <div key={index} className="p-3 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md">
                            <p className="font-medium text-gray-800 dark:text-gray-200">{title.text} <span className="text-xs text-gray-400">({title.char_count} chars)</span></p>
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1 italic">"{title.rationale}"</p>
                            <div className="mt-2 text-xs flex flex-wrap gap-x-4 gap-y-1">
                                <span title="CTR Score" className="font-mono bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 px-2 py-0.5 rounded">CTR: {title.scores.ctr_score.toFixed(1)}</span>
                                <span title="Clarity" className="font-mono text-gray-600 dark:text-gray-400">Clarity: {title.scores.clarity}</span>
                                <span title="Curiosity" className="font-mono text-gray-600 dark:text-gray-400">Curiosity: {title.scores.curiosity}</span>
                                <span title="Specificity" className="font-mono text-gray-600 dark:text-gray-400">Specificity: {title.scores.specificity}</span>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    const renderDescriptionGenerationResponse = (content: DescriptionGenerationResponse) => (
        <div className="mt-6 space-y-6">
            <div>
                ...
                <div className="whitespace-pre-wrap text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 font-sans text-sm shadow-sm">{content.description}</div>
            </div>
            {content.pinned_comment && (
                <div>
                    <h5 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">📌 Suggested Pinned Comment</h5>
                    <p className="text-gray-600 dark:text-gray-400 p-3 bg-gray-50 dark:bg-gray-800 rounded-md border dark:border-gray-700 text-sm italic">"{content.pinned_comment}"</p>
                </div>
            )}
             {content.chapters && content.chapters.length > 0 && (
                <div>
                    <h5 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">🕒 Chapters</h5>
                    <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md border dark:border-gray-700 text-sm space-y-1 text-gray-600 dark:text-gray-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {content.hashtags && content.hashtags.length > 0 && (
                    <div>
                        <h5 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">#️⃣ Hashtags</h5>
                        <div className="flex flex-wrap gap-2">
                           {content.hashtags.map(tag => <code key={tag} className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 px-2 py-1 rounded-full">{tag}</code>)}
                        </div>
                    </div>
                 )}
                  {content.keywords && content.keywords.length > 0 && (
                    <div>
                        <h5 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">🔑 Keywords</h5>
                         <div className="flex flex-wrap gap-2">
                           {content.keywords.map(kw => <code key={kw} className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 px-2 py-1 rounded-md">{kw}</code>)}
                        </div>
                    </div>
                 )}
            </div>
    ...
    const renderScriptGenerationResponse = (content: ScriptGenerationResponse) => (
        <div className="mt-6 space-y-6">
            <div>
                <h4 className="font-bold text-lg text-gray-800 dark:text-white mb-3">📜 Script Breakdown</h4>
                <div className="flex gap-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700 text-gray-600 dark:text-gray-300">
            ...
            <div className="space-y-4">
                {[...content.sections, content.midroll_cta, content.final_cta].filter(Boolean).map((section: ScriptSection | ScriptCTA, index) => (
                    <div key={index} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm transition-colors">
                        <h5 className="font-bold text-gray-800 dark:text-gray-200 capitalize mb-2 border-b dark:border-gray-700 pb-2">
                            {(section as ScriptSection).id ? (section as ScriptSection).id.replace('_', ' ') : (index === content.sections.length ? 'Mid-Roll CTA' : 'Final CTA')}
                             {(section.time_range) && <span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-2">({section.time_range})</span>}
                        </h5>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div className="space-y-3 text-gray-700 dark:text-gray-300">
                                <p><strong className="text-gray-600 dark:text-gray-400 block">🎙️ Narration:</strong> {section.narration}</p>
                                { (section as ScriptSection).beats && <p><strong className="text-gray-600 dark:text-gray-400 block"> Beats:</strong> {(section as ScriptSection).beats?.join(', ')}</p>}
                                <p><strong className="text-gray-600 dark:text-gray-400 block">📝 On-Screen Text:</strong> {section.on_screen_text}</p>
                            </div>
                            <div className="space-y-3 text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md border dark:border-gray-700">
                                <p><strong className="text-gray-600 dark:text-gray-400 block">🎬 Visuals / B-Roll:</strong> {section.visuals_broll.join('; ')}</p>
                                {(section as ScriptSection).graphics && <p><strong className="text-gray-600 dark:text-gray-400 block">🎨 Graphics:</strong> {(section as ScriptSection).graphics.join('; ')}</p>}
                                {(section as ScriptSection).sfx_music && <p><strong className="text-gray-600 dark:text-gray-400 block">🎵 SFX / Music:</strong> {(section as ScriptSection).sfx_music.join('; ')}</p>}
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <details className="bg-gray-50/50 dark:bg-gray-800/50 p-4 rounded-lg border dark:border-gray-700">
                <summary className="font-semibold text-gray-700 dark:text-gray-300 cursor-pointer">Show Alternatives</summary>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <h6 className="font-semibold text-gray-800 dark:text-gray-200">Hooks</h6>
                        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400">{content.alternatives.hooks.map((h, i) => <li key={i}>{h}</li>)}</ul>
                    </div>
                     <div>
                        <h6 className="font-semibold text-gray-800 dark:text-gray-200">CTAs</h6>
                        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400">{content.alternatives.ctas.map((c, i) => <li key={i}>{c}</li>)}</ul>
                    </div>
                     <div>
                        <h6 className="font-semibold text-gray-800 dark:text-gray-200">Title Ideas</h6>
                        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400">{content.alternatives.title_ideas.map((t, i) => <li key={i}>{t}</li>)}</ul>
                    </div>
                </div>
            </details>
        </div>
    ...
    const renderShortsIdeaResponse = (content: ShortsGenerationResponse) => (
        <div className="mt-6 space-y-6">
            {content.ideas.map((idea, index) => (
                <div key={index} className="p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm transition-all hover:shadow-lg hover:border-gray-300 dark:hover:border-gray-600">
                    <h4 className="font-bold text-xl text-gray-800 dark:text-white mb-3">{idea.title}</h4>
                    <div className="mb-4">
                        <h5 className="font-semibold text-sm text-gray-600 dark:text-gray-400 mb-2">🎣 Hooks</h5>
                        <ul className="list-none space-y-2">
                            {idea.hooks.map((hook, hookIndex) => (
                                <li key={hookIndex} className="text-gray-700 dark:text-gray-300 bg-gray-50/70 dark:bg-gray-900/50 p-3 rounded-lg border border-gray-200/80 dark:border-gray-700 text-sm italic">
                                    "{hook}"
                                </li>
                            ))}
                        </ul>
                    ...
                    <div>
                        <h5 className="font-semibold text-sm text-gray-600 dark:text-gray-400 mb-2">📝 Outline</h5>
                        <p className="text-gray-700 dark:text-gray-300 bg-gray-50/70 dark:bg-gray-900/50 p-3 rounded-lg border border-gray-200/80 dark:border-gray-700 text-sm">
                            {idea.description}
                        </p>
                    </div>
                </div>
        ...
        return (
            <div className="mt-6 space-y-4">
                <div>
                    ...
                    <p className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm font-semibold text-gray-800 dark:text-gray-200">{content.title}</p>
                </div>
                <div>
                    <div className="flex justify-between items-center mb-1">
                    ...
                    <p className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm text-sm whitespace-pre-wrap text-gray-800 dark:text-gray-200">{content.description}</p>
                </div>
                 <div>
                    <h4 className="font-bold text-lg text-gray-800 dark:text-white mb-2">Suggested Hashtags</h4>
                    <div className="flex flex-wrap gap-2">
                        {content.hashtags.map(tag => <code key={tag} className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 px-2 py-1 rounded-full">{tag}</code>)}
                    </div>
                </div>
    ...
    switch (tool.id) {
            ...
            <>
                ...
                <div className="relative w-full">
                    ...
                    <input
                      ...
                      className="peer w-full p-3 pl-12 bg-gray-50/70 dark:bg-gray-800/50 border-2 border-transparent rounded-xl focus:ring-2 focus:ring-brand-red/70 focus:border-transparent focus:bg-white dark:focus:bg-gray-800 transition-all shadow-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-600"
                    />
                </div>
                {isLoading && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Finding video...</div>}
      ...
      case 'transcript-generator':
        return (
          <>
            ...
            {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Fetching transcript... This may take a moment.</div>}
            {generatedContent && typeof generatedContent === 'string' && 
        ...
        return (
          <>
            ...
            {videoID ? (
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        ...
                        { quality: 'hqdefault', label: 'Normal Quality', resolution: '480x360' },
                         ...
                         <div key={quality} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden group transition-colors">
                            <img 
                ...
                <div className="text-center p-8 text-gray-500 dark:text-gray-400 bg-gray-50/70 dark:bg-gray-800/50 rounded-lg mt-6">
      ...
      case 'image-generator':
        return (
            <div className="space-y-6">
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <label htmlFor="prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Describe the thumbnail you want to create</label>
                         <button type="button" onClick={handleEnhancePrompt} disabled={!prompt || isEnhancing || isLoading} className="text-xs font-semibold text-brand-red hover:text-red-800 disabled:text-gray-400 dark:disabled:text-gray-600 disabled:cursor-not-allowed transition-colors">
                ...
                <div>
                     <h4 className="font-semibold mb-2 text-center text-gray-600 dark:text-gray-400">Generated Thumbnail</h4>
                     <div className="aspect-video bg-gray-100 dark:bg-gray-900 rounded-lg flex items-center justify-center text-gray-400 dark:text-gray-600 overflow-hidden shadow-inner relative group border dark:border-gray-700">
                        {isLoading ? (
      ...
      case 'thumbnail-generator':
        return (
          <div className="grid lg:grid-cols-2 gap-8 items-start">
                ...
                <div className="mt-1">
                   {originalImageUrl && imageFile ? (
                    ...
                    <div
                        ...
                        onDrop={handleDrop}
                        className={`flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-md transition-colors ${isDraggingOver ? 'border-brand-red bg-red-50 dark:bg-red-900/20' : 'border-gray-300 dark:border-gray-600'}`}
                    >
                      <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-sm text-gray-600 dark:text-gray-400">
                          <label htmlFor="image-upload" className="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-brand-red hover:text-red-700 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-brand-red">
            ...
            <div className="space-y-4">
                 <div>
                     <h4 className="font-semibold mb-2 text-center text-gray-600 dark:text-gray-400">Generated</h4>
                     <div className="aspect-video bg-gray-100 dark:bg-gray-900 rounded-lg flex items-center justify-center text-gray-400 dark:text-gray-600 overflow-hidden shadow-inner relative group border dark:border-gray-700">
                        {isLoading ? (
                            <div className="flex flex-col items-center gap-2">
                                <SpinnerIcon className="animate-spin h-6 w-6 text-brand-red"/>
      ...
      case 'x-financial-thread':
          return (
            <>
                ...
                {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Analyzing company and generating thread... This might take a moment.</div>}
      ...
      case 'x-viral-post':
          return (
            <>
                ...
                {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Crafting your viral post...</div>}
                {generatedContent && typeof generatedContent === 'string' &&
                    <div className="mt-6">
      ...
      case 'x-post-reply':
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select a Tone</h4>
                        <div className="flex flex-wrap gap-2">
                            {tones.map(tone => (
                                <button
                                    key={tone}
                                    ...
                                    className={`px-3 py-1.5 text-sm font-semibold rounded-full border-2 transition-all ${replyTone === tone ? 'bg-brand-red text-white border-brand-red' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500'}`}
                                >
                                    {tone}
                                </button>
                            ))}
                     ...
                     <div>
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select a Goal</h4>
                        <div className="flex flex-wrap gap-2">
                            {goals.map(goal => (
                                <button
                                    key={goal}
                                    ...
                                    className={`px-3 py-1.5 text-sm font-semibold rounded-full border-2 transition-all ${replyGoal === goal ? 'bg-brand-red text-white border-brand-red' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500'}`}
                                >
                                    {goal}
                                </button>
                            ))}
                ...
                {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Generating replies...</div>}
      ...
      case 'shorts-idea-generator':
        return (
            <>
                ...
                {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Generating Shorts ideas...</div>}
      ...
      case 'name-generator':
        ...
        return (
          <>
            ...
            {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Generating suggestions...</div>}
      ...
      case 'copy-assistant':
        return (
            <div className="space-y-6">
                 <div>
                    <label htmlFor="image-upload" className="block text-lg font-bold text-gray-800 dark:text-white mb-2">1. Upload Reference Thumbnail</label>
                    {originalImageUrl && imageFile ? (
                        <div className="relative group">
                        ...
                        <div className="space-y-1 text-center">
                            <svg className="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true"><path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                            <div className="flex text-sm text-gray-600 dark:text-gray-400">
                            <label htmlFor="image-upload" className="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-brand-red hover:text-red-700 focus-within:outline-none">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">The AI will analyze it and ask for your edits.</p>
                        </div>
                        </div>
                    )}
                 </div>

                {copierState.step === 'analyzing' && <div className="text-center p-4 flex items-center justify-center gap-3 text-gray-600 dark:text-gray-400"><SpinnerIcon className="w-6 h-6 mx-auto text-brand-red animate-spin"/> Analyzing Style Specs...</div>}
                ...
                {copierState.step === 'ask_questions' && copierState.analysisResult && (
                    <div className="grid lg:grid-cols-2 gap-8 items-start">
                        <div className="space-y-4">
                            ...
                            <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border dark:border-gray-700 max-h-[500px] overflow-y-auto font-mono text-xs shadow-inner text-gray-700 dark:text-gray-300">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-4 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg space-y-4 shadow-sm">
                ...
                {copierState.step === 'planning' && <div className="text-center p-4 flex items-center justify-center gap-3 text-gray-600 dark:text-gray-400"><SpinnerIcon className="w-6 h-6 mx-auto text-brand-red animate-spin"/> Generating Edit Plan...</div>}
         ...
         return (
          <>
            {creationContext.selectedTitle && (
              <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg shadow-sm">
                <p className="text-sm text-gray-600 dark:text-gray-400">Continuing with title:</p>
                <p className="font-semibold text-blue-800 dark:text-blue-300">"{creationContext.selectedTitle}"</p>
              </div>
            )}
            ...
            <div className="my-4 text-center text-gray-500 dark:text-gray-400 text-sm font-semibold">OR</div>
            ...
            {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Generating description...</div>}
      ...
      case 'chapter-generator':
         return (
          <>
            ...
            <div className="relative w-full">
                <WriteIcon className="absolute left-4 top-4 h-5 w-5 text-gray-400 dark:text-gray-500 peer-focus:text-brand-red transition-colors pointer-events-none" />
                <textarea id="transcript" value={transcript} onChange={(e) => { setTranscript(e.target.value); setCreationContext(ctx => ({...ctx, transcript: e.target.value})) }} placeholder="Paste your full video transcript here to get started..." rows={10} className="peer w-full p-4 pl-12 bg-gray-50/70 dark:bg-gray-800/50 border-2 border-transparent rounded-xl focus:ring-2 focus:ring-brand-red/70 focus:border-transparent focus:bg-white dark:focus:bg-gray-800 transition-all shadow-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-600"/>
            </div>
            {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Analyzing transcript...</div>}
      ...
      case 'script-generator':
        return (
          <>
            <label htmlFor="prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Video Topic or Idea</label>
            <div className="relative w-full">
                <WriteIcon className="absolute left-4 top-4 h-5 w-5 text-gray-400 dark:text-gray-500 peer-focus:text-brand-red transition-colors pointer-events-none" />
                <textarea id="prompt" value={prompt} onChange={(e) => { setPrompt(e.target.value); setCreationContext(ctx => ({...ctx, topic: e.target.value})) }} placeholder="e.g., How to create a successful YouTube channel from scratch in 30 days" rows={4} className="peer w-full p-4 pl-12 bg-gray-50/70 dark:bg-gray-800/50 border-2 border-transparent rounded-xl focus:ring-2 focus:ring-brand-red/70 focus:border-transparent focus:bg-white dark:focus:bg-gray-800 transition-all shadow-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-600"/>
            </div>
            {isLoading && !generatedContent && <div className="text-center p-8 text-gray-500 dark:text-gray-400">Writing your script...</div>}
      ...
      case 'shorts-video-generator':
          ...
          if (!hasApiKey) return (
            <div className="text-center p-8 bg-gray-50 dark:bg-gray-900 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-700">
                <h4 className="font-bold text-lg text-gray-800 dark:text-white">API Key Required</h4>
                <p className="text-gray-600 dark:text-gray-400 my-2">The Veo video generation model requires you to select your own API key. This is a mandatory step.</p>
                <p className="text-xs text-gray-500 dark:text-gray-500 mb-4">For more information on billing, please see the <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="text-brand-red hover:underline">billing documentation</a>.</p>
                <button
                    type="button"
          ...
          return (
             <>
                <label htmlFor="prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Video Description</label>
                <div className="relative w-full">
                    <WriteIcon className="absolute left-4 top-4 h-5 w-5 text-gray-400 dark:text-gray-500 peer-focus:text-brand-red transition-colors pointer-events-none" />
                    <textarea id="prompt" value={prompt} onChange={(e) => { setPrompt(e.target.value); setCreationContext(ctx => ({...ctx, topic: e.target.value})) }} placeholder="e.g., A cinematic drone shot of a futuristic city at sunset..." rows={4} className="peer w-full p-4 pl-12 bg-gray-50/70 dark:bg-gray-800/50 border-2 border-transparent rounded-xl focus:ring-2 focus:ring-brand-red/70 focus:border-transparent focus:bg-white dark:focus:bg-gray-800 transition-all shadow-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-600"/>
                </div>
                
                {generatedContent && typeof generatedContent === 'string' && (
                    <div className="mt-6">
                        <h4 className="font-semibold mb-2 text-gray-800 dark:text-gray-200">Generated Video</h4>
                        <video src={generatedContent} controls className="w-full rounded-lg shadow-lg aspect-[9/16] max-w-xs mx-auto bg-black" />
                        <a 
                            href={generatedContent} 
                            download="generated_shorts.mp4"
                            className="mt-4 w-full max-w-xs mx-auto flex items-center justify-center gap-2 bg-gray-800 hover:bg-gray-900 text-white font-semibold py-2 px-4 rounded-md transition-all text-sm"
                        >
                            <DownloadIcon className="w-4 h-4" />
                            Download Video
                        </a>
  ...
  const isSubmitDisabled = isLoading || (tool.id === 'shorts-video-generator' && !hasApiKey);
  const showSubmitButton = !['thumbnail-downloader'].includes(tool.id) && (tool.id !== 'copy-assistant' || copierState.step === 'ask_questions');

  // Determine appropriate submit handler
  let submitHandler = handleSubmit;
  if (tool.id === 'shorts-video-generator') submitHandler = handleVideoGenerationSubmit;
  if (tool.id === 'copy-assistant') submitHandler = handleCopierPlanGeneration;
    ...
    <div className="max-w-4xl mx-auto">
            ...
            {showSubmitButton && (
                <div className="mt-8">
                    <button
                        ...
                        className="w-full bg-brand-red hover:bg-red-700 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 ease-in-out disabled:bg-red-300 dark:disabled:bg-red-900/50 disabled:cursor-not-allowed transform hover:scale-[1.02] disabled:scale-100 flex items-center justify-center gap-3 text-lg shadow-lg hover:shadow-brand-red/30"
                    >
                        {isLoading ? (
                            <>
                                <SpinnerIcon className="w-6 h-6 animate-spin" />
                                <span>{tool.id === 'shorts-video-generator' ? 'Generating Video...' : 'Generating...'}</span>
                            </>
                        ) : (
                            <>
                                <SparklesIcon className="w-6 h-6" />
                                <span>
                                    {tool.id === 'x-video-downloader' ? 'Fetch Video' : 
                                     tool.id === 'shorts-video-generator' ? 'Generate Video' :
                                     tool.id === 'copy-assistant' ? 'Generate Edit Plan' :
                                     'Generate Content'}
                                </span>
                            </>
                        )}
                    </button>
                     {tool.id === 'shorts-video-generator' && loadingMessage && (
                        <p className="mt-4 text-center text-sm text-gray-600 dark:text-gray-400 animate-pulse">{loadingMessage}</p>
                     )}
                </div>
            )}
         </form>
        ...
        {error && (
            <div className="mt-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-start gap-3">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-red-600 dark:text-red-400 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
